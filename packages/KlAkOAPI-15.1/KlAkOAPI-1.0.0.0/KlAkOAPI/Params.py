...
from .Error import KlAkParamTypeError
...
def paramDate(value):
    if type(value) not in [str, date, datetime]:
...
def paramDouble(value):
    if type(value) is not float:  # float in python has double precision, same as double datatype in C/Java
...
def paramParams(value):
    ...
    if type(value) not in [dict, KlAkParams]:
...
class KlAkArray:
    ...
    def AddDouble(self, value):
        r""" Add a value of type float, treated as double (builtin float type in python corresponds to double in C\Java) contains paramDoule-wrapper inside """
