    ...
    def DownloadFile(self, url, path):
        ...
        while True:
            ...
            if r.status_code == http.HTTPStatus.OK or r.status_code == http.HTTPStatus.PARTIAL_CONTENT:
                ...
                if r.status_code == http.HTTPStatus.OK or len(r.content) == 0 or 'Content-Range' not in r.headers:
                    return r.status_code
                # analyze 'Content-Range' header
                m = re.match( r'bytes[\s|=](?P<range>[*]|(?P<chunk_start>\d+)[-](?P<chunk_end>\d+))/(?P<full_size>[*]|\d+)', r.headers['Content-Range'])
                if m == None:
